...
* 课堂配比：2-3 名学生共用 1 台机器人（30 人班需 10-15 台）
...
* **R5.** 支持离线语音唤醒（ESP-SR），无需联网即可激活。离线降级策略：仅支持唤醒和基础预设动作指令（见下方列表），AI 对话和云端功能不可用，LED 指示灯慢闪表示离线状态，语音播报"当前为离线模式"
  * 离线可用指令：走、停、挥手、点头、摇头、跳舞（预设动作）、左转、右转
...
* **R12.** 预留标准化扩展接口（I2C + GPIO 排针），支持即插即用添加：
  ...
  * 大尺寸显示屏模块（字幕显示、丰富表情）
  * 额外舵机扩展（更多关节/手臂）
  * **硬件规格要求**：主板必须外扩 8MB PSRAM（ESP32-S3 原生 SRAM 不足以同时运行语音唤醒 + 音频编解码 + 舵机控制+Wi-Fi），配备外置音频芯片（如 ES8311）保证课堂环境音质，内置 ≥4MB Flash 存储（用于音乐文件、离线语音模型、动作序列缓存），板载 RGB LED 状态指示灯（电源/联网/离线/错误四态）
* **R13.** 扩展模块即插即用策略：单个模块插入后固件自动识别并初始化；多模块同时使用需通过硬件 I2C 多路复用器（主板预留 TCA9548A 电路），固件通过多路复用器管理不同地址的模块。地址冲突时管理后台提示冲突模块名称和建议操作

### 3.4 动作编程与训练系统

* **R14.** 基于 Otto Blockly（Google Blockly 二次开发）提供网页端图形化编程工具，通过拖拽积木块编排动作序列。支持 28+ 预设动作积木 + 自定义舵机角度积木 + 控制流积木（循环/等待/条件）+ 传感器触发积木
...
* **R19.** 支持动作与音乐配合——学生在编程工具中上传/选择音乐，设定 BPM 和节拍点，手动将动作对齐到节拍。音乐在机器人本地播放（存储于内置 Flash，支持 MP3/WAV 格式，单首 ≤5MB），动作由机器人本地执行，避免网络延迟导致不同步
...
* AI 动作生成训练数据集（需标注 500+ 样本"描述 - 舵机序列"对）——由产品团队负责采集，MVP 前完成首轮 200+ 样本。若数据集未就绪，R18 降级为基于规则模板的动作推荐（从预设动作库按关键词匹配组合）
...
* \[Affects R14] ~~Blockly vs 自研编程工具选型~~ → 已决定采用 Otto Blockly（基于 Google Blockly），参见 R14
...
| v3.1  | 2026-04-06 | 一致性修复：增加班级规模基准（30 人/班）；编程工具选型确定为 Otto Blockly；R5 离线模式改为 LED+ 语音提示并明确离线指令列表；R12 硬件规格增加 Flash 存储和 LED 指示灯；R19 音乐存储明确为内置 Flash；删除未使用的 MCP 术语；AI 数据集标注责任方和 MVP 降级方案 |
