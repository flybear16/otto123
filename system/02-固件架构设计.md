...
| 系统堆扩展           | \~3.2MB | FreeRTOS heap\_caps\_malloc 使用 |
...
舵机引擎不作为固件内置功能，而是通过**软硬件扩展模块**实现。核心固件只提供：

* **舵机驱动接口**：PCA9685 I2C PWM 驱动，标准化的角度设置 API
* **动作执行框架**：接收 JSON 动作序列 → 插值 → 执行，与具体动作内容解耦
* **模块注册机制**：扩展模块通过 I2C 自动扫描注册，提供额外的舵机控制能力

具体的步态算法、动作库、高级编排逻辑作为可插拔模块，通过商城购买后 OTA 下发。
...
void servo_set_angles(float angles[6], uint8_t count);
void servo_release_all(void);  // 紧急停止，释放所有力矩

// 动作执行接口（接收 JSON 序列）
typedef struct {
    float angles[6];       // 目标角度
    uint16_t duration_ms;  // 插值时长
} servo_frame_t;

int action_execute(const servo_frame_t *frames, uint16_t frame_count);
int action_stop(void);  // 停止当前动作
```
...
| 扩展模块        | 功能                        | 解锁条件     |
| ----------- | ------------------------- | -------- |
| **基础步态库**   | 前进、后退、转弯、跳舞等 28+ 预设动作     | 出厂预装（免费） |
| **高级编排引擎**  | 循环/条件/传感器触发等控制流动作         | 购买高级编程模块 |
| **音乐同步模块**  | BPM 节拍对齐、音乐配合动作           | 购买音乐模块   |
| **AI 动作生成** | 自然语言描述 → 舵机序列             | 购买 AI 模块 |
| **多关节扩展**   | 支持额外 PCA9685（手臂、头部等更多自由度） | 购买扩展舵机硬件 |

### 6.4 功能许可证机制

固件在 OTA 心跳时从云端获取设备的功能许可证列表：

```
设备 → MQTT 心跳 → Business API
Business API → 查询商城服务 → 获取已购买模块列表
Business API → MQTT 下发许可证 JSON → 设备
设备 → 本地缓存许可证 → 启用对应功能模块
```
...
```json
{
  "device_id": "OTTO-001",
  "licenses": [
    {"module": "basic_gait", "expires": "2099-12-31"},
    {"module": "advanced_choreography", "expires": "2027-06-01"},
    {"module": "music_sync", "expires": null}
  ],
...
| I2C 扫描示例              | 7.2 即插即用检测  |
| OTA 示例（esp\_ota\_ops） | 8. OTA 升级流程 |
| PSRAM heap\_caps 示例   | 4. 内存管理策略   |
