...
| 功能许可证 | FeatureLicense, ModuleDefinition         | R10, R12（商城购买 → 功能解锁） |
...
erDiagram
    ...
    Device ||--o{ FeatureLicense : "has"
    ModuleDefinition ||--o{ FeatureLicense : "grants"
```
...
* **版本历史**：`version_history` JSON 字段存储最近 10 个版本的 block\_data 快照，每次保存时写入，超出时 FIFO 淘汰最旧版本
...
| ACTION\_MANUAL | 动作技能（手动组）  | 教师评分：动作数量 + 舵机维度 + 流畅度 |
| ACTION\_AI     | 动作技能（AI 组） | 教师评分：创意度 + 描述匹配度       |
...
### 3.16 功能许可证（FeatureLicense）

商城购买硬件组件后自动解锁对应软件功能。商城服务独立部署，通过 API 回调通知核心系统。

```sql
CREATE TABLE module_definition (
    id              BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    module_code     VARCHAR(64) NOT NULL COMMENT '模块编码（如 basic_gait, music_sync）',
    module_name     VARCHAR(128) NOT NULL COMMENT '模块名称',
    description     TEXT COMMENT '模块描述',
    category        VARCHAR(32) NOT NULL COMMENT '分类：builtin/purchased/hardware',
    version         VARCHAR(16) NOT NULL DEFAULT '1.0.0' COMMENT '模块版本',
    firmware_min    VARCHAR(16) COMMENT '最低固件版本要求',
    config_json     JSON COMMENT '模块配置参数（OTA 下发到固件）',
    is_active       TINYINT(1) NOT NULL DEFAULT 1,
    created_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    UNIQUE INDEX idx_module_code (module_code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='功能模块定义表';

CREATE TABLE feature_license (
    id              BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    device_id       BIGINT UNSIGNED NOT NULL COMMENT '设备 ID',
    module_code     VARCHAR(64) NOT NULL COMMENT '模块编码',
    source          VARCHAR(32) NOT NULL COMMENT '来源：purchase（购买）/ trial（试用）/ admin（管理员分配）',
    order_id        VARCHAR(64) COMMENT '商城订单号（购买来源时）',
    expires_at      DATETIME COMMENT '过期时间，NULL 表示永久',
    activated_at    DATETIME NOT NULL COMMENT '激活时间',
    created_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

    UNIQUE INDEX idx_device_module (device_id, module_code),
    INDEX idx_expires (expires_at),
    FOREIGN KEY (device_id) REFERENCES device(id) ON DELETE CASCADE,
    FOREIGN KEY (module_code) REFERENCES module_definition(module_code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='设备功能许可证表';
```

设计要点：

* **module\_definition**：定义所有可用功能模块（基础步态、高级编排、音乐同步、AI 动作生成等）
* **feature\_license**：记录每台设备已解锁的功能，由商城购买回调或管理员操作写入
* 固件 OTA 心跳时查询许可证列表，缓存到本地 Flash，按许可证启用/禁用功能模块
* 商城服务独立数据库，核心系统仅存储许可证记录，不存储订单和商品数据
...
| PRD 需求      | 数据模型支撑                                   | 关键设计点                                                               |
| ----------- | ---------------------------------------- | ------------------------------------------------------------------- |
| R16 自定义动作保存 | ProgrammingWork                          | version\_history JSON（10 版本），is\_deleted 软删除（30 天），shared\_scope 分享 |
...
| R26 安全审计    | SafetyAudit                              | 会话级检测，severity 分级通知，context\_summary 存储上下文                          |
...
* **心跳机制**：device.last\_heartbeat 字段 + 定时任务检测离线设备
