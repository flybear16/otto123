...
graph TB
    ...
    subgraph CL["云端层 Cloud Layer"]
        subgraph CL_A["已有服务（复用）"]
            AIPEN["aipen 后端<br/>智能体管理 + 设备绑定"]
            XIAOZHI["xiaozhi-esp32-server<br/>ASR → LLM → TTS 管道"]
        end
        subgraph CL_N["OTTO 新建服务"]
            BIZ["OTTO Business API<br/>业务逻辑 + 权限 + 竞赛"]
        end
        subgraph CL_S["独立微服务（解耦）"]
            SHOP["商城服务<br/>第三方开源方案"]
        end
        CL4["数据库 (MySQL + Redis)"]
    end
    ...
    FL -->|"HTTP/REST + WebSocket"| BIZ
    AIPEN -->|"WebSocket 代理"| XIAOZHI
    FWL -->|"WebSocket"| AIPEN
    FWL -->|"MQTT"| BIZ
    BIZ -->|"功能解锁查询"| SHOP
    SHOP -.->|"订单确认回调"| BIZ
    ...
    style CL_A fill:#c8e6c9
    style CL_N fill:#e1bee7
    style CL_S fill:#ffe0b2
```
...
| 固件层     | 实时控制、离线唤醒、音频处理、通信协议、OTA、功能许可证管理 | R2, R5, R7, R8, R13, R19, R28, R30    |
| 云端层（已有） | AI 语音管道、智能体管理、设备绑定              | R1, R3                                |
| 云端层（新建） | 业务逻辑、权限管控、内容推送、竞赛、功能许可证         | R6, R8, R16, R18, R20-R27, R29        |
| 云端层（独立） | 商城、订单、硬件组件购买                    | R10, R12                              |
...
| **云端后端（已有）**    | aipen 后端             | FastAPI                     | AI 语音网关、智能体管理、设备绑定、WebSocket 代理    |
|                 | xiaozhi-esp32-server | Python / WebSocket          | ASR→LLM→TTS 全链路流式管道                |
| **云端后端（新建）**    | Python / FastAPI     | Python 3.11+                | OTTO Business API（业务逻辑、权限、竞赛、内容推送） |
| **云端后端（独立）**    | 商城服务                 | 第三方开源                       | 硬件组件商城、订单管理、功能解锁（独立微服务）            |
